...
class AskcosRequest:
    smiles: str
    retro_backend_options: list[AskcosBackendOption] = field(
        default_factory=lambda: [AskcosBackendOption()]
    )
    ...
    def of(
        cls,
        smiles: str,
        retro_backend_options: list[AskcosBackendOption] | None = None,
        retro_rerank_backend: str = "relevance_heuristic",
        atom_map_backend: str = "rxnmapper",
        use_fast_filter: bool = True,
        fast_filter_threshold: float = 0.1,
...
class AskcosConfig:
    ...
    @staticmethod
    def read(path: str | Path) -> "AskcosConfig":
...
class AskcosResponse:
    def __init__(self, data: dict[str, Any]):
        self.data = data if data else {}

    @property
    def results(self) -> list[AskcosResult]:
        ...
        for d in res:
            if isinstance(d, dict):
                items.append(AskcosResult(d))
        return items


class AskcosClient:
    def __init__(self, auth: AskcosConfig):
        self.auth = auth
    ...
    def run(self, request: AskcosRequest, priority: int = 0) -> str:
        log.info(
