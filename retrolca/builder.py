...
class ProcessBuilder:
    def __init__(
        ...
        self.generated: dict[str, list[o.Process]] = {}
    ...
    def build(
        ...
        If processes for the given chemical were already generated by this
        builder, they are reused instead of generating new ones.
        ...
        """
        smiles_code = smiles.canonicalize(smiles_code)
        if generated := self.generated.get(smiles_code):
            log.info(
                "Reuse %d already generated process(es) for SMILES=%s",
                len(generated),
                smiles_code,
            )
            flow = self.flows.data.get(smiles_code)
            if category and flow and flow.category != category:
                log.warning(
                    "Ignoring category '%s' for already generated SMILES=%s; "
                    "keeping category '%s'",
                    category,
                    smiles_code,
                    flow.category,
        ...
        for reaction in reactions:
            ...
            if variant == 1:
                # Store a stub of the process and register it as the provider
                # for this product before resolving its inputs. openLCA does
                # not allow references to datasets that do not exist yet, so
                # the process (with its quantitative reference output) must be
                # persisted before a reaction cycle can link back to it.
                self.ctx.client.put(process)
            ...
            # Update the process with its resolved inputs. For the first
            # variant this overwrites the stub that was stored above.
            self.ctx.client.put(process)
            log.info("Stored process %s", process.name)
            processes.append(process)

        self.generated[smiles_code] = processes
        return processes
